...
export const baseItems: Omit<Item, 'id' | 'level' | 'price' | 'durability' | 'maxDurability'>[] = [
  ...
  // === SHIELD BUILDS - Defensive Weapons/Shields ===
  {
    name: 'Wooden Buckler',
    type: 'shield',
    rarity: 'common',
    ...
    blockChance: 20,
    handedness: 'one-handed',
    weaponSpeed: -1, // Speed penalty for blocking
    description: 'A small wooden shield perfect for deflecting attacks. Slows your attacks slightly.'
  },
  {
    name: 'Iron Shield',
    type: 'shield',
    ...
    blockChance: 30,
    handedness: 'one-handed',
    weaponSpeed: -2, // Speed penalty for blocking
    description: 'A sturdy iron shield that provides solid protection. Noticeably slows your attacks.'
  },
  {
    ...
    blockChance: 35,
    handedness: 'one-handed',
    weaponSpeed: -1, // Lighter shield, less penalty
    description: 'A defensive shield that provides excellent protection. Well-balanced design.'
  },
  {
    name: 'Steel Tower Shield',
    type: 'shield',
    rarity: 'rare',
    statBonus: { constitution: 6, strength: 2 },
    armor: 15,
    blockChance: 45,
    handedness: 'one-handed',
    weaponSpeed: -3, // Heavy shield, significant penalty
    description: 'A massive tower shield that provides excellent protection but greatly slows attacks.'
  },
  {
    ...
    criticalChance: 8,
    armor: 5,
    blockChance: 15, // Hybrid weapon/shield, lower block
    handedness: 'two-handed',
    weaponSpeed: 3,
  ...
  },
  {
    ...
    criticalChance: 10,
    armor: 8,
    blockChance: 25, // Hybrid weapon/shield, balanced
    handedness: 'two-handed',
    weaponSpeed: 5,
    description: 'Sword and shield combined, ultimate defensive weapon with no speed penalty.'
  },
  {
    name: 'Dragon Scale Shield',
    type: 'shield',
    rarity: 'epic',
    statBonus: { constitution: 8, strength: 3, intelligence: 3 },
    armor: 20,
    blockChance: 50,
    handedness: 'one-handed',
    weaponSpeed: -2, // Balanced epic shield
    description: 'Shield crafted from ancient dragon scales, nearly impenetrable but slows attacks.'
  },
  {
    name: 'Divine Aegis',
    type: 'shield',
    rarity: 'legendary',
    statBonus: { constitution: 10, strength: 5, intelligence: 5, dexterity: 3 },
    armor: 25,
    blockChance: 60,
    handedness: 'one-handed',
    weaponSpeed: -1, // Divine blessing reduces penalty
    description: 'A divine shield blessed by the gods. Divine magic minimizes the speed penalty.'
  },
...
export const getWeaponsByBuildType = (buildType: 'strength' | 'critical' | 'speed' | 'intelligence' | 'balanced' | 'tank' | 'ranged' | 'glass-cannon' | 'utility') => {
  const weaponIndices = {
    ...
    'tank': [19, 20, 21, 22, 23, 24, 25, 26], // Wooden Buckler to Divine Aegis
    'ranged': [27, 28, 29], // Hunting Bow to Dragonslayer Crossbow
    'glass-cannon': [30, 31], // Chaos Blade to Soul Reaper
    'utility': [32, 33, 34] // Lucky Charm Blade to Phase Blade
  };
...
export const generateMapSpecificInventory = (mapId: string, playerLevel: number): Item[] => {
  ...
  const mapPreferences: Record<string, {
    ...
    'greenwood_valley': {
      preferredTypes: ['weapon', 'shield', 'armor', 'boots'],
      preferredRarities: ['common', 'uncommon', 'rare'],
      levelRange: { min: Math.max(1, playerLevel - 2), max: playerLevel + 3 },
      specialItems: ['Iron Sword', 'Knight\'s Blade', 'Wooden Buckler', 'Iron Shield']
    },
    'shadowmere_swamps': {
      preferredTypes: ['weapon', 'shield', 'armor', 'accessory'],
      preferredRarities: ['uncommon', 'rare', 'epic'],
      levelRange: { min: Math.max(8, playerLevel - 1), max: playerLevel + 5 },
      specialItems: ['Poison Blade', 'Assassin\'s Dagger', 'Guardian Shield', 'Steel Tower Shield']
    },
    'crystal_caverns': {
      preferredTypes: ['weapon', 'shield', 'armor', 'accessory'],
      preferredRarities: ['rare', 'epic', 'legendary'],
      levelRange: { min: Math.max(15, playerLevel - 1), max: playerLevel + 7 },
      specialItems: ['Crystal Wand', 'Mystic Staff', 'Dragon Scale Shield', 'Philosopher\'s Stone']
    },
    'volcanic_peaks': {
      preferredTypes: ['weapon', 'shield', 'armor', 'boots'],
      preferredRarities: ['epic', 'legendary'],
      levelRange: { min: Math.max(22, playerLevel - 1), max: playerLevel + 8 },
      specialItems: ['Staff of Eternal Fire', 'Dragonbone Sword', 'Dragon Scale Shield', 'Divine Aegis']
    },
    'frozen_wastes': {
      preferredTypes: ['weapon', 'shield', 'armor', 'accessory'],
      preferredRarities: ['epic', 'legendary'],
      levelRange: { min: Math.max(32, playerLevel - 1), max: playerLevel + 10 },
      specialItems: ['Excalibur', 'Soul Reaper', 'Divine Aegis', 'Dragon Scale Shield']
    }
  };
